...
a11 <- subset(a51, a5$TofD <= 43200)
plot(a11$dur,type="l")
lines(a11$TofD/1000)
summary(a11$TofD)

a21 <- subset(a51, a5$TofD > 43200)
plot(a21$dur,type="l")
lines(a21$TofD/1000)
summary(a21$TofD)

timeofday1 <- a11$TofD - 36000
m1 <- smooth.spline(timeofday1, a11$dur, nknots=5)
plot(m1, type="l", xlab="sec", ylab=expression(phi))
timeofday2 <- a21$TofD - 46800
m2 <- smooth.spline(timeofday2, a21$dur, nknots=6)
plot(m2, type="l", xlab="sec", ylab=expression(phi))

dur.smooth <- rep(0, nrow(a51))
for (i in 1: nrow(a5)) {
  if (a51$TofD[i] <= 43200) {
    dur.smooth[i] <- predict(m1, a51$TofD[i])$y
  } else {
    dur.smooth[i] <- predict(m2, a51$TofD[i])$y
  } 
}

dur.adj <- a51$dur / dur.smooth
plot(dur.adj,type="l")
