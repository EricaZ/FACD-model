FitFACD <- function(x, id.str = "test_run", maxit = 500, portmanteau = FALSE,
                   init.param = c(2, 0.1, 0.2, 0.6), distrib = "frechet") {
  # Fits the FACD(1,1) model
  #
  # Args:
  #   x: sample
  #   id.str: character string, identifier of the run, for naming of the csv output files
  #   maxit: maximum iteration number
  #   portmanteau: if true, it will additionally output the portmanteau test statistic Q
...
#   traceparamfile <- paste(id.str, "_trace.csv", sep = "") 
  ...
  repeat {
    ...
    # Compute c1 and c2
    c <- ComputeC(r = r, err = err, distrib = "frechet")
