FitACDgeneral <- function(x, id.str, p = 1, q = 1, maxit = 100, portmanteau = FALSE,
                   init.param = list(w = 0.2, a.vec = 0.2, b.vec = 0.6, r = 2),
                   distrib = "frechet") {
...
#   traceparamfile <- paste(id.str, "_trace.csv", sep = "") 
  ...
  initparam.vec <- c(init.param$r, init.param$w, init.param$a.vec, init.param$b.vec)
  param <- init.param  # a list containing r, w, a.vec, b.vec
  ...
  repeat {
    ...
    # Compute c1 and c2
    c <- ComputeC(r = param$r, err = err, distrib = distrib)
    
    # Compute information matrix and score function
    infoscore <- ComputeISgeneral(c = c, dur = dur, gradient = gradient)
...
cat(paste("general ", distrib, " ACD with p = ", p, " and q = ", q, sep=""), "\n", sep = ",", file = finalparamfile, append = TRUE)
