library(evd)
par(mfrow=c(1,1), mar=c(2.5,2.5,2,2))
r <- 1.5
xdata <- seq(from=0, to=7, by=0.01)
theo.den <- dfrechet(xdata, loc = 0, scale = 1 / gamma(1 - 1 / r), shape = r, log = FALSE)

plot(theo.den~xdata, type="l", col="black", lwd=2, ylim=c(0, max(theo.den)), xlim=c(0,3), ann=FALSE)

r <- 2
theo.den <- dfrechet(xdata, loc = 0, scale = 1 / gamma(1 - 1 / r), shape = r, log = FALSE)
lines(theo.den~xdata, col="gray50", lwd=2)

r <- 3
theo.den <- dfrechet(xdata, loc = 0, scale = 1 / gamma(1 - 1 / r), shape = r, log = FALSE)
lines(theo.den~xdata, col="gray80", lwd=2)
